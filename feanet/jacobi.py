...
class JacobiBlock():
    ...
    def jacobi_convolution(self, u, m, nmsk, d, d_idx, term_KU=None, term_F=None, h=None, f=None, t=None, t_conn=None):
        """ 
        ...
        """
        if(self.d_mat == None):
            self.net(term_KU, term_F, h, u, d_idx, f, t, t_conn, m, nmsk)
            self.d_mat = self.compute_diagonal_matrix().to(self.device)

        u = self.reset_boundary(u, d, d_idx)
        residual = self.net(term_KU, term_F, h, u, d_idx, f, t, t_conn, m, nmsk)
        u_new = self.omega/self.d_mat*residual + u
        return nmsk*self.reset_boundary(u_new, d, d_idx)